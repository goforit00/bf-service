        ...
        try {
            if(params != null && params.size() > 0) {
                ...
                while(respStr.hasNext()) {
                    ...
                    e.append((String)entry.getKey()).append("=").append(URLEncoder.encode((String)entry.getValue(), charset)).append("&");
                }
            ...
            RequestConfig e2 = RequestConfig.custom().setSocketTimeout(20000).setConnectTimeout(20000).setConnectionRequestTimeout(20000).setStaleConnectionCheckEnabled(true).build();
            httpClient = HttpClients.createDefault();
            httpGet.setConfig(e2);
            httpResp = httpClient.execute(httpGet);
            int statusCode2 = httpResp.getStatusLine().getStatusCode();
            if(statusCode2 != 200) {
                throw new RuntimeException("request URL=" + url + " fail, status code=" + statusCode2);
            }

            String respStr2 = EntityUtils.toString(httpResp.getEntity(), charset);
            EntityUtils.consume(httpResp.getEntity());
            LOGGER.info("HttpUtil-doGet request url={}, response={}", url, respStr2);
            entry1 = respStr2;
        } catch (Exception var14) {
            LOGGER.error("HttpUtil-doGet request fail, url={}, params={}", url, params);
